  ...
  return <main className="content-page library-page">
    ...
    <section className="library-grid">
      {visible.map((strain) => <article className="strain-tile" key={strain.id}>
        <div className="tile-top"><span>{strain.project?.isGBSProject ? "GBS project" : strain.strainType?.replace("-", " ") ?? "unknown"}</span><button onClick={() => favorite(strain.id)} aria-label={`${progress.strains[strain.id]?.favorite ? "Remove" : "Add"} ${strain.name} ${progress.strains[strain.id]?.favorite ? "from" : "to"} favorites`}><Heart size={17} fill={progress.strains[strain.id]?.favorite ? "currentColor" : "none"} /></button></div>
    ...
    {!visible.length && <div className="empty-state"><h2>No archive records match</h2><p>Try a broader search or clear the family filter.</p></div>}
  </main>;
}
