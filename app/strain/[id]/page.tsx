...
import { AromaSection, BreedingValueSection, CultivationSection, EffectsSection, HistorySection, LineageSection, PhenotypeSection, SourcesSection } from "@/app/components/card/sections";
...
export default async function StrainDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="content-page detail-page">
    ...
    <section className="detail-hero">
      ...
      <div><p className="eyebrow">{strain.project?.isGBSProject ? "GBS breeding project" : "Permanent archive record"}</p><h1>{strain.name}</h1><p>{strain.breeder ?? "Breeder not documented"}</p><DocumentationTierBadge tier={strain.documentationTier} /></div>
    </section>
    ...
    <div className="detail-sections"><LineageSection strain={strain} /><AromaSection strain={strain} /><EffectsSection strain={strain} /><CultivationSection strain={strain} /><PhenotypeSection strain={strain} /><BreedingValueSection strain={strain} /><HistorySection strain={strain} /><SourcesSection strain={strain} /></div>
  </main>;
}
